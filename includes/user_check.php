...
try {
    ...
    $user = Cache::remember($cacheKey, function () use ($pdo, $userId) {
        $stmt = $pdo->prepare("SELECT Rolle, SekundarRolle, Name, ZeitpruefungFreigabe FROM Benutzer WHERE BenutzerID = ?");
        $stmt->execute([$userId]);
    ...
    $_SESSION['zeitpruefung_freigabe'] = (int)($user['ZeitpruefungFreigabe'] ?? 0);
