...
$fahrerSql = "SELECT CONCAT(Vorname, ' ', Nachname) AS Name, Urlaubstage FROM Fahrer WHERE FahrerID = ?";
$fahrerParams = [$fahrer_id];
if ($companyId !== null) {
    $fahrerSql .= " AND company_id = ?";
    $fahrerParams[] = $companyId;
}
$stmtFahrer = $pdo->prepare($fahrerSql);
$stmtFahrer->execute($fahrerParams);
...
$gesamtSql = "
    ...
    WHERE FahrerID = ? AND Datum BETWEEN ? AND ?
";
$gesamtParams = [$fahrer_id, $start_date, $end_date];
if ($companyId !== null) {
    $gesamtSql = "
        ...
        WHERE u.FahrerID = ? AND u.Datum BETWEEN ? AND ? AND f.company_id = ?
    ";
    $gesamtParams[] = $companyId;
}
$stmtGesamt = $pdo->prepare($gesamtSql);
$stmtGesamt->execute($gesamtParams);
