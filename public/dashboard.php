...
try {
    ...
    $activeDriversParams = [];
    if ($companyId !== null) {
        $activeDriversSql .= " AND (f.company_id = :company_id OR v.company_id = :company_id)";
        $activeDriversParams['company_id'] = $companyId;
    }
    $activeDriversSql .= " ORDER BY firmenname, sfa.anmeldung DESC";
    $stmt = $pdo->prepare($activeDriversSql);
    $stmt->execute($activeDriversParams);
...
$hasFahrerBirthdateColumn = tableHasColumn($pdo, 'Fahrer', 'birth_date');
$hasZentraleBirthdateColumn = tableHasColumn($pdo, 'mitarbeiter_zentrale', 'geburtsdatum');

if ($hasFahrerBirthdateColumn) {
    try {
        $stmt_fahrer = $pdo->prepare(
    ...
    $missingBirthdateMessage = 'Spalte "birth_date" existiert nicht in der Tabelle Fahrer.';
