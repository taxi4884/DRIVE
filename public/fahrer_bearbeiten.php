 ...
 */
function fetchDriver(PDO $pdo, int $fahrerId, ?int $companyId): ?array
{
    $sql = 'SELECT * FROM Fahrer WHERE FahrerID = ?';
    $params = [$fahrerId];
    if ($companyId !== null) {
        $sql .= ' AND company_id = ?';
        $params[] = $companyId;
 ...
 */
function fetchGesamtUrlaubstage(PDO $pdo, int $fahrerId, ?int $companyId): int
{
    $sql = 'SELECT Urlaubstage FROM Fahrer WHERE FahrerID = ?';
    $params = [$fahrerId];
    if ($companyId !== null) {
        $sql .= ' AND company_id = ?';
        $params[] = $companyId;
...
// Fahrer-Daten laden
$fahrer = fetchDriver($pdo, $fahrer_id, $companyId);
if (!$fahrer) {
    header('Location: fahrer.php');
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $shiftTrackingEnabled = isset($_POST['shift_tracking_enabled']) ? 1 : 0;

    $updateSql = 'UPDATE Fahrer SET
            Vorname = ?,
            Nachname = ?,
...
$gesamt_urlaubstage = fetchGesamtUrlaubstage($pdo, $fahrer_id, $companyId);
        ...
        <form action="fahrer_bearbeiten.php?id=<?= htmlspecialchars($fahrer_id) ?>" method="POST">
			...
			<div class="form-section">
				...
				<input type="text" id="personalnummer" name="personalnummer" value="<?= htmlspecialchars($fahrer['Personalnummer'] ?? '') ?>">
                                ...
                                <label for="shift_tracking_enabled"><i class="fa-solid fa-clock"></i> Schichtstunden-Tracking aktivieren:</label>
                                <div style="margin: 0.3rem 0 1rem 0;">
                                    <input type="checkbox" id="shift_tracking_enabled" name="shift_tracking_enabled" value="1" style="width:auto; margin-right:8px; transform:none;" <?= (int)($fahrer['shift_tracking_enabled'] ?? 0) === 1 ? 'checked' : '' ?>>
                                    <span>Beim Login Schichten aus Taxidaten speichern</span>
                                </div>
			</div>
