<?php
echo "Start der Datei erreicht.<br>";
exit();

register_shutdown_function(function() {
    $error = error_get_last();
    if ($error !== null) {
        echo "<pre style='color:red;'>SHUTDOWN ERROR:\n";
        print_r($error);
        echo "</pre>";
    }
});
...
/* ==========================================================
 * Konfiguration für FMS-Endpoint
 * ========================================================== */
const FMS_ENDPOINT = 'https://4884gateway.de/fms'; // Ziel-URL

/* ==========================================================
 * Einladungen prüfen & versenden (5 neue + 3 alte Standard)
 * ========================================================== */
function checkUndVersendeEinladungen($termin, $maxEinladungen = 8){
    ...
    if ($freiNeu > 0 && $gesamtFrei > 0) {
        ...
        $stmt->bindValue(':termin', $termin, PDO::PARAM_STR);
...
/* ==========================================================
 * Einladung verschicken und Termin stempeln
 * ========================================================== */
function versendeEinladung($id, $termin){
    ...
    $praxistagdatum  = DateTime::createFromFormat('Y-m-d', $termin)->format('d.m.Y');

    /* E‑Mail verschicken (versand.php) */
    if (sendInvitation($id, $vorname, $email, $praxistagdatum)) {
        /* Wichtig: Einladungs‑Stempel auf den TERMIN setzen (nicht CURDATE) */
        $update = $pdo->prepare(
            "UPDATE schulungsteilnehmer
                SET letzte_einladung = :termin
              WHERE id = :id"
        );
        $update->execute([':termin' => $termin, ':id' => $id]);
...
/* ==========================================================
 * Helfer: Nummernzähler, Code, FMS-Sendung
 * ========================================================== */
function getNextFahrerNummer(PDO $pdo): int {
    $pdo->beginTransaction();
    try {
        // Annahme: genau eine Zeile in fahrernummer_counter
        ...
        $next = ((int)$row['letzte_nummer']) + 1; // Start 6999 → erste Ausgabe 7000
...
function sendSetFahrer(array $payload): array {
    ...
    return ['ok'=>$ok, 'http_code'=>$code, 'response'=>$response ?? '', 'error'=>$err ?: null];
}

/* ==========================================================
 * Auto-Entsperrung abgelaufener Sperrfristen
 * ========================================================== */
$freigabeQuery = "
    UPDATE schulungsteilnehmer 
...
/* ==========================================================
 * POST: Termin speichern (einzeln oder global + Massenversand)
 * ========================================================== */
...
/* ==========================================================
 * POST: Rückmeldungen zurücksetzen
 * ========================================================== */
...
/* ==========================================================
 * Statistik auslesen (bestehend aus deiner Logik)
 * ========================================================== */
$statsQuery = "
    SELECT 
        COUNT(*) AS gesamt,
...
/* ==========================================================
 * POST: Schulungsergebnis speichern (inkl. FMS-SETFAHRER bei bestanden)
 * ========================================================== */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['set_bestanden'])) {
    ...
    try {
        if ($status === 1) {
            // Teilnehmer hat bestanden ⇒ Fahrer anlegen & an FMS senden
            try {
                // 1) Teilnehmerdaten holen – für Namen (optional)
                $stmt = $pdo->prepare("
					SELECT
						vorname,
						nachname,
				...
				");
                $stmt->execute([':id' => $id]);
                $tn = $stmt->fetch(PDO::FETCH_ASSOC) ?: ['vorname'=>null, 'nachname'=>null];

                // 2) Nächste Nummern holen (FAHRER_NR und DISPLAYNUMMER gleich)
                $nextNr = getNextFahrerNummer($pdo); // nutzt fahrernummer_counter (Start 6999)

				// 3) Feldmapping & Formate
				$plz  = $tn['postleitzahl'] ?? ($tn['plz'] ?? '');
				$gbd  = '';
				if (!empty($tn['geburtsdatum'])) {
					// angenommen im DB-Format YYYY-MM-DD
					$dt = DateTime::createFromFormat('Y-m-d', (string)$tn['geburtsdatum']);
					if ($dt) { $gbd = $dt->format('d.m.Y'); }
				}
				// Handynummer ohne Leerzeichen
				$mobil = isset($tn['handynummer']) ? preg_replace('/\s+/', '', (string)$tn['handynummer']) : '';

				// 4) Payload bauen (Datum DD.MM.YYYY wie im Beispiel)
				$payload = [
					'SETFAHRER' => [
						'FAHRER_NR'            => (string)$nextNr,
						'DISPLAYNUMMER'        => (string)$nextNr,
						'ANMELDECODE'          => sixDigitCode(),
						'UNTERNEHMER_NR'       => '1200',
						'Vertragsbeginn'       => date('d.m.Y'),

						// Person & Adresse (neu)
						'VORNAME'              => $tn['vorname']    ?? '',
						'NACHNAME'             => $tn['nachname']   ?? '',
						'STRASSE'              => $tn['strasse']    ?? '',
						'HAUSNUMMER'           => $tn['hausnummer'] ?? '',
						'PLZ'                  => $plz,
						'ORT'                  => $tn['ort']        ?? '',
						'GEBURTSDATUM'         => $gbd,
                ...
                if (!$res['ok']) {
                    // NICHT löschen – Admin-Info sammeln
                    $_SESSION['message'] = sprintf(
                        "Fahrer-Anlage FEHLGESCHLAGEN (HTTP %d): %s %s",
                        $res['http_code'],
                        $res['error'] ? "cURL: ".$res['error'].';' : '',
                        $res['response'] ? 'Antwort: '.$res['response'] : ''
                    );
                } else {
                    // 5) Erfolgreich → Teilnehmer löschen
                    $stmt = $pdo->prepare("DELETE FROM schulungsteilnehmer WHERE id = :id");
                    $stmt->execute([':id' => $id]);
                    $_SESSION['message'] = "Teilnehmer hat bestanden, Fahrer angelegt (Nr. {$nextNr}) und an FMS gesendet.";
                }
            } catch (Throwable $e) {
                $_SESSION['message'] = 'Fehler bei Fahrer-Anlage/Versand: ' . $e->getMessage();
            }
        } else {
...
/* ==========================================================
 * Nächster Termin mit bereits verschickten Einladungen
 * ========================================================== */
$nextQuery = "
    SELECT MIN(schulungstermin)
      FROM schulungsteilnehmer
     WHERE schulungstermin >= CURDATE()
       AND letzte_einladung = schulungstermin   -- wichtig: wir stempeln jetzt exakt den Termin
";
$nextTermin = $pdo->query($nextQuery)->fetchColumn();

/* Rückmeldungen für diesen Termin zählen */
...
<body>
    ...
    <main>
        ...
        <?php if ($nextTermin): ?>
            <?php $dateDe = (new DateTime($nextTermin))->format('d.m.Y'); ?>
