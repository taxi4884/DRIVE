...
// Sicherstellen, dass Projektmanagement-Freigabe existiert
try {
    $stmt = $pdo->prepare("SHOW COLUMNS FROM Benutzer LIKE 'ProjectmanagementFreigabe'");
    $stmt->execute();
    if (!$stmt->fetch(PDO::FETCH_ASSOC)) {
        $pdo->exec("ALTER TABLE Benutzer ADD COLUMN ProjectmanagementFreigabe TINYINT(1) NOT NULL DEFAULT 0");
    }
} catch (PDOException $e) {
    // stilles Failover, Seite bleibt nutzbar
}
