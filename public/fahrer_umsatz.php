...
$fahrerSql .= " ORDER BY Personalnummer IS NULL, Personalnummer ASC, Nachname ASC";
$stmtAlleFahrer = $pdo->prepare($fahrerSql);
$stmtAlleFahrer->execute($fahrerParams);
...
$fahrerIds = array_column($alleFahrer, 'FahrerID');
if ($fahrer_id && !in_array((int) $fahrer_id, $fahrerIds, true)) {
    $fahrer_id = $alleFahrer[0]['FahrerID'] ?? null;
}
...
$fahrerInfoSql = "SELECT CONCAT(Vorname, ' ', Nachname) AS Name FROM Fahrer WHERE FahrerID = ?";
$fahrerInfoParams = [$fahrer_id];
if ($companyId !== null) {
    $fahrerInfoSql .= " AND company_id = ?";
    $fahrerInfoParams[] = $companyId;
}
$stmtFahrer = $pdo->prepare($fahrerInfoSql);
$stmtFahrer->execute($fahrerInfoParams);
...
";
$umsatzGesamtParams = [$fahrer_id, $start_date, $end_date];
if ($companyId !== null) {
    $umsatzGesamtSql = "
        SELECT SUM(u.TaxameterUmsatz + u.OhneTaxameter) AS GesamtUmsatz
        FROM Umsatz u
        JOIN Fahrer f ON u.FahrerID = f.FahrerID
        WHERE u.FahrerID = ? AND u.Datum BETWEEN ? AND ? AND f.company_id = ?
    ";
    $umsatzGesamtParams[] = $companyId;
}
$stmtGesamt = $pdo->prepare($umsatzGesamtSql);
$stmtGesamt->execute($umsatzGesamtParams);
...
$umsatzSql = "
	...
	ORDER BY Datum ASC
";
$umsatzParams = [$fahrer_id, $start_date, $end_date];
if ($companyId !== null) {
    $umsatzSql = "
        SELECT 
            u.UmsatzID,
    ...
    ";
    $umsatzParams[] = $companyId;
}
$stmtUmsatz = $pdo->prepare($umsatzSql);
$stmtUmsatz->execute($umsatzParams);
$umsatzDaten = $stmtUmsatz->fetchAll(PDO::FETCH_ASSOC);


// NEU-Schema Mapping (Icon + Doppelklick-Modal)
$neuByDate = [];
$neuDetailsById = [];
try {
    ...
    if ($hasNeuHead && $hasNeuDetails) {
        $neuStmt = $pdo->prepare('SELECT id, datum FROM driver_abrechnung_neu WHERE fahrer_id = ? AND datum BETWEEN ? AND ? ORDER BY datum ASC');
        $neuStmt->execute([$fahrer_id, $start_date, $end_date]);
        $neuRows = $neuStmt->fetchAll(PDO::FETCH_ASSOC);

        $neuIds = [];
        foreach ($neuRows as $nr) {
            $d = (string)($nr['datum'] ?? '');
            if ($d !== '') {
                $neuByDate[$d] = (int)$nr['id'];
                $neuIds[] = (int)$nr['id'];
            }
        }

        if (!empty($neuIds)) {
            $in = implode(',', array_fill(0, count($neuIds), '?'));
            $detStmt = $pdo->prepare("SELECT abrechnung_neu_id, datum, ride_idx, zeitpunkt, von_text, nach_text, brutto, kategorie FROM driver_abrechnung_neu_fahrten WHERE abrechnung_neu_id IN ($in) ORDER BY abrechnung_neu_id ASC, ride_idx ASC");
            $detStmt->execute($neuIds);
            foreach ($detStmt->fetchAll(PDO::FETCH_ASSOC) as $dr) {
                $nid = (int)$dr['abrechnung_neu_id'];
                if (!isset($neuDetailsById[$nid])) $neuDetailsById[$nid] = [];
                $neuDetailsById[$nid][] = $dr;
            }
        }
    }
} catch (Throwable $e) {
...
$neuDetailsJson = json_encode($neuDetailsById, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
if ($neuDetailsJson === false) { $neuDetailsJson = '{}'; }
?>
<?php
$title = 'Fahrer Umsatz';
...
?>
    ...
    <style>
        ...
        #neuFahrtenContent table {
            font-size: 0.92rem;
            table-layout: fixed;
            width: 100%;
        }
        #neuFahrtenContent th, #neuFahrtenContent td {
            white-space: nowrap;
            overflow: hidden;
            text-overflow: ellipsis;
        }
        #neuFahrtenContent td:nth-child(3),
        #neuFahrtenContent td:nth-child(4) {
            text-align: left;
            white-space: normal;
        }
        .btn-primary {
        ...
        <section>
            ...
            <table>
                <thead>
                    <tr>
                        <th title="Neues Abrechnungsschema"><i class="fas fa-bolt"></i></th>
                        ...
                        <th><i class="fas fa-cash-register"></i> Bar berechnet</th>
                        <th><i class="fas fa-piggy-bank"></i> Eingezahlt</th>
                        <th><i class="fas fa-coins"></i> Offen</th>
                        <th>Aktionen</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if (!empty($umsatzDaten)): ?>
                        <?php foreach ($umsatzDaten as $umsatz): ?>
                            <?php $neuId = $neuByDate[$umsatz['Datum']] ?? null; ?>
                            <tr <?= $neuId ? 'data-neu-id="' . (int)$neuId . '" style="cursor:zoom-in;"' : '' ?>>
                                <td>
                                    <?php if ($neuId): ?>
                                        <span title="Neues Schema vorhanden (Doppelklick für Fahrten)"><i class="fas fa-bolt" style="color:#f59e0b;"></i></span>
                                    <?php else: ?>
                                ...
                                <?php
                                    $barBerechnet = (float)$umsatz['TaxameterUmsatz']
                                        + (float)$umsatz['OhneTaxameter']
                                        - (float)$umsatz['Kartenzahlung']
                                        - (float)$umsatz['Rechnungsfahrten']
                                        - (float)$umsatz['Krankenfahrten']
                                        - (float)$umsatz['Gutscheine']
                                        - (float)$umsatz['Alita']
                                        - (float)$umsatz['TankenWaschen']
                                        - (float)$umsatz['SonstigeAusgaben'];
                                    if ($barBerechnet < 0) { $barBerechnet = 0.0; }
                                    $eingezahlt = (float)($umsatz['EingezahltBetrag'] ?? 0);
                                    $offen = $barBerechnet - $eingezahlt;
                                    if ($offen < 0) { $offen = 0.0; }
                                ?>
                                <td><strong><?= number_format($barBerechnet, 2, ',', '.') ?></strong></td>
                                <td>
                                    <?= number_format($eingezahlt, 2, ',', '.') ?>
                                    <?php if (!empty($umsatz['EingezahltAm'])): ?>
                                        <div style="font-size:0.75em;color:#666;"><?= htmlspecialchars(date('d.m.Y H:i', strtotime($umsatz['EingezahltAm']))) ?><?php if (!empty($umsatz['EingezahltVon'])): ?> · <?= htmlspecialchars($umsatz['EingezahltVon']) ?><?php endif; ?></div>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <?php if ($offen > 0.009): ?>
                                        <span style="color:#b45309;font-weight:600;">€ <?= number_format($offen, 2, ',', '.') ?></span>
                                    <?php else: ?>
                                        <span style="color:green;font-weight:600;">0,00</span>
                                    <?php endif; ?>
                                </td>
                                <td>
									...
									<?php endif; ?>
                                    <?php if ($offen > 0.009): ?>
                                        <form method="POST" action="markiere_eingezahlt.php" style="display:inline;">
                                            <input type="hidden" name="umsatzid" value="<?= $umsatz['UmsatzID'] ?>">
                                            <input type="hidden" name="fahrer_id" value="<?= htmlspecialchars($fahrer_id) ?>">
                                            <input type="hidden" name="start_date" value="<?= htmlspecialchars($start_date) ?>">
                                            <input type="hidden" name="end_date" value="<?= htmlspecialchars($end_date) ?>">
                                            <input type="hidden" name="betrag" value="<?= htmlspecialchars(number_format($offen,2,'.','')) ?>">
                                            <button type="submit" class="action-btn" onclick="return confirm('Offenen Betrag als eingezahlt markieren?');">
                                                <i class="fas fa-money-check-alt"></i> Eingezahlt
                                            </button>
                                        </form>
                                    <?php else: ?>
                                        <span style="color: green; font-size: 0.9em;"><i class="fas fa-check-circle"></i> Eingezahlt</span>
                                    <?php endif; ?>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="15">Keine Umsätze für den gewählten Zeitraum.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
    ...
    <div class="modal" id="neuFahrtenModal" style="display:none;">
        <h2>Fahrten (neues Abrechnungsschema) – <span id="neuFahrtenDatum"></span></h2>
        <div id="neuFahrtenContent">Keine Daten.</div>
        <div style="display:flex;justify-content:flex-end;gap:8px;margin-top:10px;">
            <button type="button" onclick="closeNeuFahrtenModal()"><i class="fas fa-times"></i> Schließen</button>
        </div>
    </div>
    ...
    <script>
        ...
        const neuDetailsMap = <?= $neuDetailsJson ?>;

        function openNeuFahrtenModal(neuId) {
            const modal = document.getElementById('neuFahrtenModal');
            const content = document.getElementById('neuFahrtenContent');
            const datumEl = document.getElementById('neuFahrtenDatum');
            if (!modal || !content || !datumEl) return;

            const rides = neuDetailsMap[String(neuId)] || neuDetailsMap[neuId] || [];
            const datum = (rides[0] && rides[0].datum) ? rides[0].datum : '';
            datumEl.textContent = datum || '';
            if (!rides.length) {
                content.innerHTML = '<p>Keine Fahrten gefunden.</p>';
            } else {
                let html = '<table class="table table-sm table-striped"><thead><tr><th>#</th><th>Zeit</th><th>Von</th><th>Nach</th><th>Kategorie</th><th>Betrag</th></tr></thead><tbody>';
                rides.forEach((r, i) => {
                    const t = (r.zeitpunkt || '').toString();
                    const tz = t ? t.substring(11,16) : '';
                    const betrag = Number(r.brutto || 0).toLocaleString('de-DE', {minimumFractionDigits:2, maximumFractionDigits:2});
                    html += '<tr>'+
                        '<td>' + (Number(r.ride_idx ?? i) + 1) + '</td>'+
                        '<td>' + (tz || '-') + '</td>'+
                        '<td>' + (r.von_text || '') + '</td>'+
                        '<td>' + (r.nach_text || '') + '</td>'+
                        '<td>' + (r.kategorie || '') + '</td>'+
...
(function(){
  ...
  function openById(neuId){
    ...
    if (!rides.length) {
      ...
      rides.forEach((r, i) => {
        const t = (r.zeitpunkt || '').toString();
        const tz = t ? t.substring(11,16) : '-';
        const b = Number(r.brutto || 0).toLocaleString('de-DE', {minimumFractionDigits:2, maximumFractionDigits:2});
        html += '<tr><td>' + (Number(r.ride_idx ?? i) + 1) + '</td><td>' + tz + '</td><td>' + (r.von_text || '') + '</td><td>' + (r.nach_text || '') + '</td><td>' + (r.kategorie || '') + '</td><td>' + b + ' €</td></tr>';
      });
      html += '</tbody></table>';
      content.innerHTML = html;
    }
  ...
  window.closeNeuFahrtenModal = function(){
    if(!modal) return;
    modal.classList.remove('active');
    modal.style.display = 'none';
  };

  document.querySelectorAll('tr[data-neu-id]').forEach(function(row){
    row.addEventListener('dblclick', function(){
      const id = row.getAttribute('data-neu-id');
      if(id) openById(id);
    });
  });
})();
