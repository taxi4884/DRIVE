...
if ($companyId !== null) {
    $where_clause .= " AND f.company_id = :company_id";
}
...
if ($companyId !== null) {
    $stmt_umsatz_schicht->bindValue(':company_id', $companyId, PDO::PARAM_INT);
}
...
$sql_umsatz_fahrer_monate = "SELECT f.Vorname, f.Nachname, DATE_FORMAT(u.Datum, '%Y-%m') AS monat,
                             ...
                             FROM Umsatz u
                             JOIN Fahrer f ON u.FahrerID = f.FahrerID
                             WHERE f.Status IN ('aktiv', 'Aktiv')$companyClause
...
if ($companyId !== null) {
    $stmt_umsatz_fahrer_monate->bindValue(':company_id', $companyId, PDO::PARAM_INT);
}
...
if ($companyId !== null) {
    $stmt_arbeitstage->bindValue(':company_id', $companyId, PDO::PARAM_INT);
}
...
if ($companyId !== null) {
    $stmt_wochentage->bindValue(':company_id', $companyId, PDO::PARAM_INT);
}
...
if ($companyId !== null) {
    $stmt_effizienz->bindValue(':company_id', $companyId, PDO::PARAM_INT);
}
...
if ($companyId !== null) {
    $stmt_durchschnitt_umsatz_monat->bindValue(':company_id', $companyId, PDO::PARAM_INT);
}
...
if ($companyId !== null) {
    $stmt_fahrer_unter_264->bindValue(':company_id', $companyId, PDO::PARAM_INT);
}
